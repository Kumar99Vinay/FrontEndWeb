...
//		 System.out.println(users);
		 ...
		 try {
			 if (f.length() == 0) {
                 ...
                 users.add(new User("Jay1", "Diu", "9090909090", "jaykumar1", "J@ykumar99", "900000",90000));
 				for(User user : users) oos.writeObject(user);
 				oos.close();
             }else {
            	 MyObjectOutputStream oos = null;
                 oos = new MyObjectOutputStream(new FileOutputStream(outputFile,true));
                 users.add(new User("Jay2", "Diu", "9090909090", "jaykumar2", "J@ykumar99", "900000",90000));
 				for(User user : users) {
...
//		String regex = digits+ alphabets+ specialChars+ WhiteSpace ;
		...
		try {
			...
			if(password  == "") {throw new BankExceptions("Must Enter Password"); }
			else {
